...
function parseStoragePublicUrl(url) {
  try {
    ...
    if (alt && alt[1] && alt[2]) return {
      bucket: alt[1],
      objectPath: decodeURIComponent(alt[2])
    };
    // relative object key (no leading slash)
    if (noQ.startsWith('resumes/')) return {
      bucket: 'resumes',
      objectPath: noQ.replace(/^resumes\//, '')
    };
    if (noQ.startsWith('documents/')) return {
      bucket: 'documents',
      objectPath: noQ.replace(/^documents\//, '')
    };
  } catch (_) {}
  return null;
}
...
async function generatePdfFromMarkdown(md, title) {
  ...
  for(let i = 0; i < lines.length; i++){
    ...
    if (bullet) {
      ...
      for (const word of words){
        ...
        if (font.widthOfTextAtSize(t, 11) > max) {
          flush();
          cur = word;
        } else {
          cur = t;
...
// OpenAI helpers --------------------------------------------------------------
async function openaiUploadFile(fileBytes, fileName, mime) {
  ...
  const arrBuf = new Uint8Array(uint8).slice().buffer;
  ...
  if (!res.ok) {
    ...
    throw {
      ...
      openai: openaiErr,
      raw: errText,
      message: `OpenAI file upload failed: ${res.status}`
    };
  }
...
async function safeDownload(bucket, rawPath) {
  const candidates = Array.from(new Set([
    cleanPath(rawPath),
    cleanPath(decodeURIComponent(rawPath)),
    cleanPath(encodeURI(rawPath)),
    cleanPath(rawPath.replace(/^resumes\//, '')),
...
async function fetchResumeFileById(resumeId) {
  // New table: resumes
  const { data: r1 } = await supabase.from('resumes').select('file_url, file_name').eq('id', resumeId).maybeSingle();
  if (r1?.file_url) {
    ...
    return {
      bytes: arr,
      filename: r1.file_name || 'resume',
      mime
    };
  }
  // Legacy table: candidate_resumes
  const { data: r2 } = await supabase.from('candidate_resumes').select('file_path, name').eq('id', resumeId).maybeSingle();
  if (r2?.file_path) {
    ...
    const initialBucket = parsed?.bucket || (/^documents\//.test(r2.file_path) || /\/documents\//.test(r2.file_path) ? 'documents' : undefined) || (/^resumes\//.test(r2.file_path) || /\/resumes\//.test(r2.file_path) ? 'resumes' : undefined) || 'documents' // default guess: most legacy uploads used 'documents'
    ;
    ...
    const bucketsToTry = initialBucket === 'documents' ? [
      'documents',
      'resumes'
    ] : [
      'resumes',
    ...
    for (const b of bucketsToTry){
      try {
        ...
        return {
          bytes: arr,
          filename: r2.name || 'resume',
          mime: 'application/octet-stream'
        };
      } catch (e) {
        lastErr = e;
        console.warn(`[tailor-cv] download attempt failed bucket='${b}' path='${objectPath}':`, e?.message || e);
      }
    }
...
async function fetchJobById(jobId) {
  const { data, error } = await supabase.from('jobs').select('title, company, description').eq('id', jobId).single();
...
async function openaiExtractTextFromFile(fileBytes, fileName, mime) {
  const uint8 = fileBytes instanceof ArrayBuffer ? new Uint8Array(fileBytes) : fileBytes;
  ...
  if (!res.ok) {
    ...
    throw {
      ...
      openai: openaiErr,
      raw: errText,
      message: `OpenAI extract failed: ${res.status}`
    };
  }
  const json = await res.json();
...
async function openaiGenerateTailoredMarkdown(resumeText, jobTitle, companyName, jobDescription) {
...
SOURCE RESUME TEXT (authoritative content to preserve):\n${resumeText}`;
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'Respond ONLY with the Markdown resume. No code fences.'
        },
        {
          role: 'user',
          content: prompt
  ...
  if (!res.ok) {
    ...
    throw {
      ...
      openai: openaiErr,
      raw: errText,
      message: `OpenAI tailor failed: ${res.status}`
    };
  }
  const json = await res.json();
...
function markdownSanitizeForPdf(md) {
  if (!md) return md;
...
serve(async (req)=>{
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const mdKey = `${base}.md`;
      const { error: upErr } = await supabase.storage.from('tailored-resumes').upload(pdfKey, pdfBytes, {
        contentType: 'application/pdf',
        upsert: false
      });
      if (upErr) throw new Error(`Failed to upload tailored pdf: ${upErr.message}`);
      const mdBytes = new TextEncoder().encode(mdClean);
      const { error: upMdErr } = await supabase.storage.from('tailored-resumes').upload(mdKey, mdBytes, {
        contentType: 'text/markdown; charset=utf-8',
        upsert: false
      });
      if (upMdErr) throw new Error(`Failed to upload markdown: ${upMdErr.message}`);
      const { data: row, error: insErr } = await supabase.from('tailored_resumes').insert({
        user_id: userId || null,
      ...
      return new Response(JSON.stringify({
        ...
        downloadUrl: supabase.storage.from('tailored-resumes').getPublicUrl(pdfKey).data.publicUrl,
        markdownUrl: supabase.storage.from('tailored-resumes').getPublicUrl(mdKey).data.publicUrl,
        score: 85
      }), {
        headers: {
          ...corsHeaders,
    ...
    if (!resumeText && body.resumeId) {
      ...
      console.log(`[tailor-cv] requestId=${requestId} resume fetched filename='${f.filename}' mime='${f.mime}' size=${f.bytes.byteLength || 0}`);
    ...
    if (upErr) throw new Error(`Failed to upload tailored pdf: ${upErr.message}`);
    const { error: upMdErr } = await supabase.storage.from('tailored-resumes').upload(mdKey, new TextEncoder().encode(mdClean), {
    ...
    const openaiStage = error?.stage;
    const openaiStatus = error?.status;
    const openaiError = error?.openai;
    const openaiRaw = error?.raw;
    return new Response(JSON.stringify({
      success: false,
      error: info.userMessage,
      errorCode: info.code,
      technicalError: info.message,
      stack: info.stack,
      requestId,
      openaiStage,
      openaiStatus,
